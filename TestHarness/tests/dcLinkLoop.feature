...
Feature: DC-link voltage loop holds the bus against source disturbances
  ...
  settling 100 ms after the event, inverter current limit 20.2 A.
...
Scenario: Source step - PV and wind come up
  Source current steps 0 to 10 A at 50 ms. The bus must stay inside the
  overshoot envelope and be back in the tolerance band 100 ms later.
  Given inputs
    * Vdc_ref = const(700)
    * i_src = step(0 -> 10 @ 50ms)
...
Scenario: Cloud transient - irradiance collapses
  Source current drops 10 to 2 A at 200 ms. Assertions start after 200 ms so
  the model's own start-up transient is not measured as a failure.
  Given inputs
    * Vdc_ref = const(700)
    * i_src = step(10 -> 2 @ 200ms)
  When simulate for 500ms in Normal mode
  Then outputs
...
Scenario: Reference step - operator raises the setpoint
  Setpoint steps 700 to 750 V at 200 ms under a steady 10 A source.
...
Scenario: Overload - source exceeds what the inverter can absorb
  Source steps to 60 A, three times the 20 A inverter limit. Voltage
