...
fprintf("source: models/pv/solarsimulink.slx   dV = %.2f V / %.0f ms = %.0f V/s\n", ...
        P0.ctrl.dV, P0.ctrl.Ts*1e3, P0.ctrl.slewRate);
fprintf("%s\n", repmat('=', 1, 96));
...
if ~isempty(rail)
    lines(end+1) = sprintf( ...
        ...
        "    duty -- and duty stops at %.2f. With the load fixed at 4.08 ohm the operating point\n" + ...
...
if ~isempty(slow)
    ...
    lines(end+1) = sprintf( ...
        ...
        "    P&O moves the voltage reference by %.2f V every %.0f ms, so it can slew %.0f V/s\n" + ...
        "    and no faster. This is a rate limit, not a tuning error -- raising dV or\n" + ...
        "    shortening the perturbation period are the only levers.", ...
        strjoin(string({slow.scenario}), ", "), worst, P.spec.reacquireTime, ...
        P.ctrl.dV, P.ctrl.Ts*1e3, P.ctrl.slewRate);
end
...
if ~isempty(rip)
    lines(end+1) = sprintf( ...
        ...
        "    dV = %.2f V. Reducing it lowers ripple and slows reacquisition -- the two specs\n" + ...
        "    pull in opposite directions and dV is the single knob between them.", ...
        P.spec.powerRipplePct, strjoin(string({rip.scenario}), ", "), P.ctrl.dV);
end
